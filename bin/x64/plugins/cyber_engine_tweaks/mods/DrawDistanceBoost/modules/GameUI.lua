...
local function safeObserve(className, funcName, callback)
    -- CET Observe throws if the target method doesn't exist (game version differences).
    -- We silence those errors to keep GameUI usable across patches.
    local ok = pcall(Observe, className, funcName, callback)
    return ok
end
	...
	if required[GameUI.Event.Menu] and not initialized[GameUI.Event.Menu] then
		...
		for menuScenario, menuItemEvents in pairs(menuItemListeners) do
			for menuEvent, menuItem in pairs(menuItemEvents) do
				safeObserve(menuScenario, menuEvent, function()
