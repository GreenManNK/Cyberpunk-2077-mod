...
function drink:new(id, interactionPosition, workspotPosition, workspotRotation)
	local o = {}
    ...
    o.devicePath = "base\\gameplay\\devices\\arcade_machines\\drink.ent"
    o.name = GetLocalizedText("LocKey#38969")
    ...
    -- Slide / Enter data
    o.slideTime = 0.75
    o.slideCameraRot = EulerAngles.new(0, -23.5, -6.6) -- Camera rot target to match animation entry
    o.slideCameraPos = Vector4.new(0, -0.055, -0.032, 0) -- Camera pos target to match animation entry
    ...
    -- Sounds
    o.enterSounds = {
        {name = "q001_sc_01_v_sits_on_stool", delay = 0.5}
    }
    o.exitSounds = {
        {name = "q001_sc_01_v_stands_up_from_stool", delay = 0}
    }
    ...
    o.drinkAttached = false
    o.drinkID = 36775116ULL
    o.entityID = nil
...
function drink:init() -- Setup basic info, create world interaction and hub
...
function drink:registerObservers()
...
function drink:setupEntry() -- Entry situation
    -- Entry dialog UI:
    ...
    interaction.callbacks[1] = function()
        ...
        self.slide = tween.new(self.slideTime + 0.1, {x = pos.x, y = pos.y, z = pos.z, roll = rot.roll, pitch = rot.pitch, yaw = rot.yaw}, {x = self.workspotPosition.x, y = self.workspotPosition.y, z = self.workspotPosition.z, roll = self.workspotRotation.roll, pitch = self.workspotRotation.pitch, yaw = yawGoal}, tween.easing.outQuad)
...
function drink:drinkSequence()
    Game.GetWorkspotSystem():SendForwardSignal(GetPlayer())
    utils.playAudio("q001_sc_01_v_starts_drinking_whiskey")

    Cron.After(0.632, function()
        self.drinkAttached = true
    end)

    Cron.After(4.23, function()
        self.drinkAttached = false
    end)

    Cron.After(4.83, function()
        Game.GetWorkspotSystem():SendJumpCommandEnt(GetPlayer(), 2, true)
        self.enableCamera = true
        self:setupExit()
        utils.applyStatus("BaseStatusEffect.Drunk")
    end)
end

function drink:setupExit() -- Exit situation
    -- Exit dialog UI:
    local exitChoice = interaction.createChoice(self.exitText, TweakDBInterface.GetChoiceCaptionIconPartRecord(self.exitIcon))
    local drinkChoice = interaction.createChoice("[" .. GetLocalizedText("LocKey#510") .. "]", TweakDBInterface.GetChoiceCaptionIconPartRecord("ChoiceCaptionParts.UseIcon"))
    self.hub = interaction.createHub(self.name, {drinkChoice, exitChoice})
    interaction.setupHub(self.hub)
    interaction.showHub()
...
function drink:update(dt)
    ...
    local drink = Game.FindEntityByID(entEntityID.new({hash = self.drinkID}))
    if not drink then return end

    GameObjectEffectHelper.StartEffectEvent(drink, "d_liquid_whiskey_glass_mid", true, worldEffectBlackboard.new())

    if self.drinkAttached then
        local _, pos = GetPlayer():FindComponentByName("ItemAttachmentSlots"):GetSlotTransform("WeaponRight")
        Game.GetTeleportationFacility():Teleport(drink, pos:GetWorldPosition():ToVector4() , pos:GetOrientation():ToEulerAngles())
    end
end
