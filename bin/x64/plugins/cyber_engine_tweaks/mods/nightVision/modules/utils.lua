...
function miscUtils.addEffect(name, path)
    CName.add(name)
    local effects = GetPlayer():FindComponentByName("fx_status_effects").effectDescs
    local custom = entEffectDesc.new()
    custom.effectName = name
    custom.effect = path
    table.insert(effects, custom)
    GetPlayer():FindComponentByName("fx_status_effects").effectDescs = effects
end

function miscUtils.playSound(name, target, mult)
    local m = mult or 1
