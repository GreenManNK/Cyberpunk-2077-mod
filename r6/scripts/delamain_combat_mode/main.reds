...
protected cb func OnDriverCombatWeaponTypeChange(newWeaponType: Int32) -> Bool {
  ...
  if Equals(t"Vehicle.v_standard2_villefort_cortes_delamain_player", vehicleObj.GetRecordID()) {
    // FTLog(s"Delamain Combat Mode: Vehicle.v_standard2_villefort_cortes_delamain_player");
    if newWeaponType == 101 {
      let animFeature: ref<AnimFeature_PartData> = new AnimFeature_PartData();
      animFeature.state = 1;
      animFeature.duration = 1.3;
      AnimationControllerComponent
        .ApplyFeatureToReplicate(vehicleObj, n"seat_back_right_window", animFeature);
      // AnimationControllerComponent
